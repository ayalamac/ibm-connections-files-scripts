 ...
 */
 ...
 function getCommunityName__SPNS__(data) {
	 var title = $(data).find('feed').find('title');
	 return 'Files for ' + $(title[0]).text() + ' Community'; // the first one is the name of the Community
 }
...
/**
* getFolders__SPNS__ returns just the folders for a Community
*/
function getFolders__SPNS__() {
	var resourceURL = "/wps/proxy/https/" + connectionsHost__SPNS__ + "/files/basic/api/communitycollection/" + communityUuid + "/feed?sK=updated&sO=dsc&category=collection&type=all";
	// console.log('getting Files from ', resourceURL);
	$.ajax({
		method: "GET",
		url: resourceURL,
		async: false,
...
* uuid = the uuid of a folder within a Community
*/
function getContentsOfFolder__SPNS__(uuid) {
	var resourceURL = "/wps/proxy/https/" + connectionsHost__SPNS__ + "/files/basic/api/collection/" + uuid + "/feed?sK=modified&sO=dsc&sC=all&acls=true&collectionAcls=true&includeNotification=true&includePolicy=true&category=all&includeAncestors=true&pageSize=50";
	$.ajax({
		method: "GET",
		url: resourceURL,
		async: false,
...
/**
* getFiles__SPNS__ calls Connections to get the files (not the folders) in a Community
* it then calls processContentsOfFolder__SPNS__ with the result of the call
*/
function getFiles__SPNS__() {
	var resourceURL = "/wps/proxy/https/" + connectionsHost__SPNS__ + "/files/basic/api/communitycollection/" + communityUuid + "/feed?sK=modified&sO=dsc&sC=document&acls=true&collectionAcls=true&pageSize=50";
	$.ajax({
		method: "GET",
		url: resourceURL,
		async: false,
		...
		.done(function( data ) {
			// console.log(data);
			$('#communityName').html(getCommunityName__SPNS__(data));
...
function processContentsOfFolder__SPNS__(data, parent) {
	...
	$(data).find('entry').each(function(index,item){
		...
		if ( !foundMatch) {
			filesTreeData__SPNS__.push({ "id" : uuid, "parent" : parent, "text" : formatDetailsLink__SPNS__(item) , "icon" : getIcon__SPNS__(title), "li_attr": {"openlink":link}, "openlink": link });
		}
	});
}
