...
function getFolders__SPNS__() {
	var resourceURL = "/wps/proxy/https/" + connectionsHost__SPNS__ + "/files/basic/api/collections/feed";
	...
	$.ajax({
		...
		.fail(function( data ) {
			console.log('getFolders failed!! ', data);
...
function getContentsOfFolder__SPNS__(uuid) {
	...
	$.ajax({
		...
		.fail(function( data ) {
			console.log('getContentsOfFolder failed!! ', data);
			$('#filesTree__SPNS__').html("Error retrieving data from Connections");
	});
}
...
function getFiles__SPNS__() {
	var resourceURL = "/wps/proxy/https/" + connectionsHost__SPNS__ + "/files/basic/api/myuserlibrary/feed?includePath=true&ps=50";
	$.ajax({
		method: "GET",
		url: resourceURL,
		async: false,
		...
		.fail(function( data ) {
			console.log('getFiles failed!! ', data);
			$('#filesTree__SPNS__').html("Error retrieving data from Connections");
	});
}
...
function getContentsOfSubfolder__SPNS__(uuid) {
	...
	$.ajax({
		...
		.fail(function( data ) {
			console.log('getContentsOfSubfolder failed!! ', data);
			$('#filesTree__SPNS__').html("Error retrieving data from Connections");
	});
}
